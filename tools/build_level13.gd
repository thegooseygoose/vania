extends SceneTree
## Builds res://Level13.tscn — a REGULAR (overground) blank stage. 300 tiles wide, two solid
## rows of ground (atlas 0). Editable: paint your own content on the layers in the Godot editor.
##   godot --headless --path <proj> -s tools/build_level13.gd
const T := 16
const FLOOR := 13
const SID := 0
const LW := 300
...
func _init() -> void:
	...
	var err := ResourceSaver.save(packed, "res://Level13.tscn")
	print("build_level13: saved (err=%d) LW=%d cells=%d" % [err, LW, terrain.get_used_cells().size()])
	quit()
