extends SceneTree
# measure how much of the open space is reachable from start (4-neighbour), current Level29
...
func _run():
	...
	if sy<0: sy=0
	var opent=0
	for i in solid.size(): if solid[i]==0: opent+=1
	...
	var r=0
	for i in reach.size(): if reach[i]==1: r+=1
	print("current reachable=%d / open=%d (%.0f%%)"%[r,opent,100.0*r/max(1,opent)])
	quit()
