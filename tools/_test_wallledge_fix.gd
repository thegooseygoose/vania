extends SceneTree
## Real-engine test: jump from the wall-ledge (151, x160-164) up to the new
## intermediate (149, x163-166), then up to the mid-ledge (146, x166-169).
var m
...
func _run() -> void:
	...
	var hops := [
		{"from_x": 162 * 16 + 8, "from_y": (151 - 1) * 16 + 8, "to_x": 165 * 16 + 8, "to_y": (149 - 1) * 16 + 8},
		{"from_x": 165 * 16 + 8, "from_y": (149 - 1) * 16 + 8, "to_x": 168 * 16 + 8, "to_y": (146 - 1) * 16 + 8},
	]
	var all_ok := true
	for h in hops:
