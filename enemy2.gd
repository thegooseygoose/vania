...
class_name Enemy2
## ENEMY2 -- the alternate enemy script (starts as an exact copy of enemy.gd). Pick it on the main menu
## ("AI: ENEMY2.GD") to play with these movements instead of enemy.gd. Edit freely; enemy.gd is untouched.
...
const TURRET_FIRE := 1.0        # Turret: fires a shot straight AT the player every second (was 0.5s -- firing rate halved)
