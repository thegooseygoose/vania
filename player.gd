...
const WALL_COYOTE := 0.28
const WALL_CLIMB_X := 120.0      # holding INTO the wall: small nudge away so you re-stick and can jump the SAME wall again
const WALL_CLIMB_LOCK := 0.085
var grappling := false
...
func _update_alive(delta: float) -> void:
	...
	if jump_key and on_floor and not jump_held and not riding:   # no jumping while on the bike
		...
		last_wall_dir = wd
		wall_coyote = WALL_COYOTE if into else 0.0   # a climb hop keeps the wall "warm" so the next hop chains; a kick across clears it (no mid-air re-jumps)
		_wall_hop = true
		jump_held = true
